...
def get_sparse_laplacian(edge_index, edge_weight, num_nodes, normalization='none'):
    # 使用PyTorch原生操作替代torch_scatter
    row, col = edge_index[0], edge_index[1]

    # 计算度数：使用scatter_add的PyTorch原生实现
...
def get_masked_laplacian(edge_index, edge_weight, num_nodes, normalization='none'):
    # Use PyTorch native operations instead of torch_scatter
    row, col = edge_index[0], edge_index[1]

    # Calculate degree: use PyTorch native implementation of scatter_add
    deg = torch.zeros(num_nodes, dtype=edge_weight.dtype, device=edge_weight.device)
    deg.scatter_add_(0, row, edge_weight)
