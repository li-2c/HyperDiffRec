...
class GeneralRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataloader):
        ...
        # load encoded features here with modality completion support
        ...
        if not config['end2end'] and config['is_multimodal_model']:
            ...
            # Initialize modality completion module
            from utils.modality_completion import create_completion_module
            self.completion_module = create_completion_module(config)

            # Setup completion model with feature dimensions
            v_dim = self.v_feat.shape[1] if self.v_feat is not None else None
            t_dim = self.t_feat.shape[1] if self.t_feat is not None else None
            self.completion_module.setup_completion_model(v_dim=v_dim, t_dim=t_dim)

            # Apply modality completion
            self.v_feat, self.t_feat = self.completion_module.complete_features(self.v_feat, self.t_feat)
