...
from utils.multi_metric_evaluator import MultiMetricEvaluator, enhanced_early_stopping
...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def __init__(self, config, model):
        ...
        # 🔥 初始化多指标评估器
        self.multi_metric_evaluator = MultiMetricEvaluator(config)
    ...
    def _valid_epoch(self, valid_data):
        ...
        # 🔥 使用多指标评估器获取评估分数
        if self.multi_metric_evaluator.enable:
            valid_score = self.multi_metric_evaluator.get_evaluation_score(valid_result)
        else:
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                # 🔥 使用增强的early stopping逻辑
                if self.multi_metric_evaluator.enable:
                    self.best_valid_result, self.cur_step, stop_flag, update_flag = enhanced_early_stopping(
                        valid_result, self.best_valid_result, self.multi_metric_evaluator,
                        self.cur_step, max_step=self.stopping_step)
                    # 更新best_valid_score用于兼容性
                    self.best_valid_score = self.multi_metric_evaluator.get_evaluation_score(self.best_valid_result)
                else:
                ...
                # 🔥 使用多指标格式化结果
                if self.multi_metric_evaluator.enable:
                    valid_result_output = '📊 VALID result: \n' + self.multi_metric_evaluator.format_result_summary(valid_result)
                else:
                ...
                if verbose:
                    ...
                    # Format test result using multi-metric evaluator
                    if self.multi_metric_evaluator.enable:
                        test_result_output = '🎯 TEST result: \n' + self.multi_metric_evaluator.format_result_summary(test_result)
                    else:
                ...
                if update_flag:
                    ...
                    if not self.multi_metric_evaluator.enable:
                        self.best_valid_result = valid_result
